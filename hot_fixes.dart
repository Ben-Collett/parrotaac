// save this as fix_flutter_tts_cmake.dart in your project root
import 'dart:io';

void main() async {
  // Path to the plugin's CMakeLists.txt
  final cmakeFile = File(
    'flutter/ephemeral/.plugin_symlinks/flutter_tts/windows/CMakeLists.txt',
  );

  if (!cmakeFile.existsSync()) {
    print('❌ Could not find flutter_tts CMakeLists.txt at: ${cmakeFile.path}');
    exit(1);
  }

  print('🔧 Overriding flutter_tts CMakeLists.txt...');

  const fixedContent = r'''
...
execute_process(
  COMMAND ${NUGET_EXE}
  ARGS install Microsoft.Windows.CppWinRT
...
''';
  ...
  print('✅ CMakeLists.txt has been patched successfully.');
}
